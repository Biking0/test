# -*- coding: utf-8 -*-
"""
Created on Mon Jun 29 10:56:09 2020
...
def importExcelToMysql(cur, path):
    ...
    ###
        ...
        valuestr = [sqlstr[0], str(sqlstr[1]), str(sqlstr[2]), sqlstr[3],str(sqlstr[4]), str(sqlstr[5])]
 
        # 将每行数据存到数据库中
        cur.execute("insert into etl_control_dependence(son_id, son_path_name, son_etl_name, parent_id,parent_path_name,parent_etl_name) values(%s, %s, %s, %s,%s,%s)", valuestr)
 
# 输出数据库中内容
def readTable(cursor):
    # 选择全部
    cursor.execute("select * from etl_control_dependence")
...
if __name__ == '__main__':
    # 和数据库建立连接
    conn = pymysql.connect('localhost', 'zhw', 'haiwang', charset='utf8')
    # 创建游标链接
    cur = conn.cursor()
    ...
    # 将 excel 中的数据导入 数据库中
    importExcelToMysql(cur, "./yilai.xlsx")
    readTable(cur)
