...
do
    ...
    then
		...
		# 添加HDFS目标表目录写入权限
        #Write_Log_File "hadoop fs -chmod 777 hdfs://$DATA_TO:8020/user/hive/warehouse/${source_dir} >> ${Log_File}"
        #hadoop fs -chmod 775 hdfs://$DATA_TO:8020/user/hive/warehouse/${source_dir} >> ${Log_File}
		
		## 修改所属用户
		#Write_Log_File "hadoop fs -chown ocetl:ocdp hdfs://$DATA_TO:8020/user/hive/warehouse/${source_dir} >> ${Log_File}"
		#hadoop fs -chown ocetl:ocdp hdfs://$DATA_TO:8020/user/hive/warehouse/${source_dir} >> ${Log_File}
		...
		beeline -u "jdbc:hive2://ocdpdn80:10000/default" -n ocetl -p demo -e "alter table ${TODIR} add partition(${partition_dir})"
